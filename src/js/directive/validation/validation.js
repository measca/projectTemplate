...
function Validation(form, vm, validationObj, validationVM, doms, success, error) {
  ...
  function init(validate, validationObj, validationVM) {
    ...
    //处理验证关系
    validationVM.$set("isPass", true); //是否通过
    validationVM.$set("isChange", false); //内容是否被改变
    validationVM.$set("reset", function () {
      validate.reset();
    });
    validationVM.$set("initVal", function () {
      validate.initVal();
    });
    validationVM.$set("validateAll", function () {
    ...
    for (var key in validationModel) {
      ...
      var attrName = "model." + key;
      validationVM.$set(attrName, check);
      watchData(validationVM, directiveModels, attrName);
  ...
  function watchData(vm, directiveModels, attrName) {
    var vm = vm;
    var directiveModels = directiveModels;
    vm.$watch(attrName + ".isPass", function (v) {
      setTimeout(function () {
        var state = true;
    ...
    //监察所有check 的 isChange 变化
    vm.$watch(attrName + ".isChange", function (v) {
      setTimeout(function () {
        var state = false;
        $.each(directiveModels, function (i, v) {
          if (v.check.isChange) {
...
export function install(Vue) {
  Vue.directive('validation', {
    ...
    bind: function () {
      ...
      var doms = {};
      $(":text,:password,:radio,:checkbox,:file,select,:submit", this.el).each(function () {
        var el = this;
      ...
      this.vm["$" + configName] = validationVM;
